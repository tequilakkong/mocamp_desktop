...
function mouseenterFunc() {
  if (!this.parentNode.classList.contains("active")) {
    //여기서 this는 마우스를 올린 .gnb_tit //자바스크립트 .classList.contains = 제이쿼리 hasClass
    for (let i = 0; i < links.length; i++) {
...
function select(divE1, buttonE1) {
  Array.from(divE1.children).forEach((v) => v.classList.remove("selected"));
...
var selArea = document.querySelectorAll(".field_inner .select_area");
var popBox = document.querySelectorAll(".pop_box .pop");
var popCloseBtn = document.querySelector(".pop_close_btn");
...
for (let i = 0; i < selArea.length; i++) {
  ...
  selArea[i].addEventListener("click", showPopFunc); //click
}

function popEnterFunc() {
  if (!this.parentNode.classList.contains("active")) {
    //여기서 this는 마우스를 올린 .selArea
    for (let i = 0; i < selArea.length; i++) {
...
function moveSlide(num) {
  //반드시 숫자가 넘어와야 일을 하는 함수이므로 num을 변수로 넣어줌
  slides.style.left = -num * (slideWidth + slideMargin) + "px";
  currentIdx = num;
}

nextBtn.addEventListener("click", function () {
  //addEvnetListener('어떤 이벤트?','할 일')
